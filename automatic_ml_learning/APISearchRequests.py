...
def data_setup():
    if os.path.exists(config.out_dir + "Cleaned_Lab_Names.csv") and os.path.exists(
            config.out_dir + "Cleaned_Specimen_Names.csv"):
...
def get_match(term, searchType, tgt, AuthClient):
    ...
    while cont and count < maxCount:
        try:
            if searchType == "exact":
                content_endpoint = "/rest/search/current?string=" + term + "&searchType=exact&ticket=" + AuthClient.getst(
                    tgt)
            if searchType == "word":
                content_endpoint = "/rest/search/current?string=" + term + "&searchType=words&ticket=" + AuthClient.getst(
                    tgt)
    ...
    if(len(items['result']['results']) > 0 ):
        print("first found",items["result"]['results'][0]["name"] )
    return items["result"]
                ...
                if len(jsonData["results"]) == 0:
                    ...
                    if len(jsonData["results"]) == 0:
                        for term in token.split(sep=" "):
                            ...
                            if len(jsonData["results"]) != 0 and len(master[token]) < 3:
                                print("+++++++++++++++++++++")
                                print("added", jsonData["results"][0]["name"] )
                                print("++++++++++++++++++++++")
