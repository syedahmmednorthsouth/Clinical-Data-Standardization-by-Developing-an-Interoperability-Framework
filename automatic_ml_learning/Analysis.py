...
def transform_and_filter_data():
    dat = get_data()
    dat.to_json('transform/first_data.csv')
    loinc_synonyms = get_loinc_synonyms()
    loinc_synonyms.to_csv('transform/loincsynonyms.csv')

    if config.print_status == 'Y':
        print('Filtering and Factorizing Data')
    feature_col_number = config.num_cuis
    X_unfiltered = dat.copy()
    X_unfiltered = X_unfiltered.merge(loinc_synonyms, how='left', left_on=config.loinc_col, right_on='LOINC_NUM').drop(
        'LOINC_NUM', axis=1)
    X_unfiltered.loc[X_unfiltered.LOINC_KEY.isnull(), 'LOINC_KEY'] = X_unfiltered.loc[
        X_unfiltered.LOINC_KEY.isnull(), config.loinc_col]
    ...
    X_unfiltered = X_unfiltered.merge(cumulct_orig_loinc, how='left', left_on=config.loinc_col,
                                      right_on=config.loinc_col)
    ...
    X_unfiltered[[config.loinc_col, config.units, 'TestNameMapJW', 'TestNameMapLV', 'SpecimenMapLV',
                  ...
                  'PredictedSystemJW', 'PredictedSystemLV', 'LOINC_KEY']] = X_unfiltered[
        [config.loinc_col, config.units,
    ...
    loinc_coder = preprocessing.LabelEncoder().fit(
        pd.concat([X_unfiltered[config.loinc_col], X_unfiltered.LOINC_KEY]).unique())

    X_unfiltered[cols_to_tranform] = X_unfiltered[cols_to_tranform].apply(
        lambda x: label_encoder_dict[x.name].transform(x))

    X_unfiltered[[config.loinc_col, 'LOINC_KEY']] = X_unfiltered[[config.loinc_col, 'LOINC_KEY']].apply(
        lambda x: loinc_coder.transform(x))
    ...
    unknowns = pd.concat([unknowns, site_cts[site_cts.SiteCountByLOINCGrp <= config.min_sites_per_loinc_key].drop(
        ['SiteCountByOrigLOINC', 'SiteCountByLOINCGrp'], axis=1)])
...
def get_site_splits():
    ...
    for i in range(n_splits):
        ...
        while site_ct < len(working_list) and (len(test_sites) == 0 or
                                               X0[X0[config.site].isin(test_sites)].shape[0] / N_rows < (
                                                       (1 / n_splits) - 0.005)):
...
def get_indices():
    ...
    print("site_splits", site_splits)

    print(len(site_splits))

    for j in range(len(site_splits)):
        np.random.seed(seed=seed)
        test_ind.append(X0[X0[config.site].isin(site_splits[j])].index)
        print("j : ", j)
        print("test_ind", len(test_ind[j]))

        tune_test_ind.append(np.random.choice(test_ind[j], replace=False, size=int(len(test_ind[j]) * 0.5)))
...
def rf_hyperopt_train_test(rf_params):
    ...
    for i in range(config.n_splits):
        ...
        X_train = X0.iloc[np.concatenate(tune_train_ind[:i] + tune_train_ind[i + 1:])].drop(
            [config.site, config.loinc_col], axis=1)
        y_train = X_train.pop('LOINC_KEY')
        X_test = X0.iloc[np.concatenate(tune_test_ind[:i] + tune_test_ind[i + 1:])].drop(
            [config.site, config.loinc_col], axis=1)
...
def run_cv(unknowns_analysis, site_splits, rf_final_parms, ovr_final_parms):
    ...
    nb_clf_accuracy = np.zeros(N_SPLITS)
    nb_clf_f1_weighted = np.zeros(N_SPLITS)
    nb_clf_f1_macro = np.zeros(N_SPLITS)
    nb_clf_f1_micro = np.zeros(N_SPLITS)
    nb_clf_preds = []
    nb_clf_unk_preds = []


    knn_accuracy = np.zeros(N_SPLITS)
    knn_f1_weighted = np.zeros(N_SPLITS)
    knn_f1_macro = np.zeros(N_SPLITS)
    knn_f1_micro = np.zeros(N_SPLITS)
    knn_preds = []
    knn_unk_preds = []

    dt_accuracy = np.zeros(N_SPLITS)
    dt_f1_weighted = np.zeros(N_SPLITS)
    dt_f1_macro = np.zeros(N_SPLITS)
    dt_f1_micro = np.zeros(N_SPLITS)
    dt_preds = []
    dt_unk_preds =[]

    svc_accuracy = np.zeros(N_SPLITS)
    svc_f1_weighted = np.zeros(N_SPLITS)
    svc_f1_macro = np.zeros(N_SPLITS)
    svc_f1_micro = np.zeros(N_SPLITS)
    svc_preds = []
    svc_unk_preds = []

    logicR_accuracy = np.zeros(N_SPLITS)
    logicR_f1_weighted = np.zeros(N_SPLITS)
    logicR_f1_macro = np.zeros(N_SPLITS)
    logicR_f1_micro = np.zeros(N_SPLITS)
    logicR_preds = []
    logicR_unk_preds = []

    voting_clf_accuracy = np.zeros(N_SPLITS)
    voting_clf_f1_weighted = np.zeros(N_SPLITS)
    voting_clf_f1_macro = np.zeros(N_SPLITS)
    voting_clf_f1_micro = np.zeros(N_SPLITS)
    voting_clf_preds = []
    voting_clf_unk_preds = []

    # X_tr = X0
    # X_tr.to_csv("trainx.csv")
    # y_tr = X_tr.pop('LOINC_KEY')
    # y_tr.to_csv("trainy.csv")
    #
    # print("start feature selection")
    # # The following code is for selecting the best k features from all features
    # selection = SelectKBest(chi2, k=10).fit(X_tr, y_tr)
    # x_new = selection.transform(X_tr)
    #
    ...
    for i in range(N_SPLITS):
        ...
        X_unk_test = unknowns_analysis[unknowns_analysis[config.site].isin(site_splits[i])].drop([config.site,
                                                                                                  config.loinc_col],
                                                                                                 axis=1)
        X_unk_test.to_csv("testunkx.csv")
        ...
        plot_learning_curves(X_train, y_train, X_test, y_test, clf=rf, train_marker='o', test_marker='^',
                             scoring='misclassification error', print_model=True, style='fivethirtyeight',
                             legend_loc='upper left', suppress_plot=True)
        plt.show()
        ...
        knn_preds.append(y_pred)
        y_unk_pred = knn_clf.predict(X_unk_test)
        knn_unk_preds.append(y_unk_pred)

        plot_learning_curves(X_train, y_train, X_test, y_test, clf=knn_clf, train_marker='o', test_marker='^',
                             scoring='misclassification error', print_model=True, style='fivethirtyeight',
                             legend_loc='upper left')
        plt.show()

        nb_clf_accuracy[i] = accuracy_score(y_test, y_pred)
        nb_clf_f1_weighted[i] = f1_score(y_test, y_pred, labels=knn_clf.classes_, average='weighted')
        nb_clf_f1_macro[i] = f1_score(y_test, y_pred, labels=knn_clf.classes_, average='macro')
        nb_clf_f1_micro[i] = f1_score(y_test, y_pred, labels=knn_clf.classes_, average='micro')

        if config.print_status == 'Y':
            print("decision tree  CV: ", i + 1)
        X_train = X0[~X0[config.site].isin(site_splits[i])].drop([config.site, config.loinc_col], axis=1)
        y_train = X_train.pop('LOINC_KEY')
        X_test = X0[X0[config.site].isin(site_splits[i])].drop([config.site, config.loinc_col], axis=1)
        y_test = X_test.pop('LOINC_KEY')
        ...
        y_unk_pred = dt_clf.predict(X_unk_test)
        dt_unk_preds.append(y_unk_pred)

        plot_learning_curves(X_train, y_train, X_test, y_test, clf=dt_clf, train_marker='o', test_marker='^',
                             scoring='misclassification error', print_model=True, style='fivethirtyeight',
                             legend_loc='upper left')
        plt.show()

        dt_accuracy[i] = accuracy_score(y_test, y_pred)
        dt_f1_weighted[i] = f1_score(y_test, y_pred, labels=dt_clf.classes_, average='weighted')
        dt_f1_macro[i] = f1_score(y_test, y_pred, labels=dt_clf.classes_, average='macro')
        dt_f1_micro[i] = f1_score(y_test, y_pred, labels=dt_clf.classes_, average='micro')


        if config.print_status == 'Y':
            print("Naive Bayes CV: ", i + 1)
        X_train = X0[~X0[config.site].isin(site_splits[i])].drop([config.site, config.loinc_col], axis=1)
        y_train = X_train.pop('LOINC_KEY')
        X_test = X0[X0[config.site].isin(site_splits[i])].drop([config.site, config.loinc_col], axis=1)
        y_test = X_test.pop('LOINC_KEY')
        ...
        nb_clf_preds.append(y_pred)
        y_unk_pred = nb_clf.predict(X_unk_test)
        nb_clf_unk_preds.append(y_unk_pred)

        plot_learning_curves(X_train, y_train, X_test, y_test, clf=nb_clf, train_marker='o', test_marker='^',
                             scoring='misclassification error', print_model=True, style='fivethirtyeight',
                             legend_loc='upper left')
        plt.show()

        nb_clf_accuracy[i] = accuracy_score(y_test, y_pred)
        nb_clf_f1_weighted[i] = f1_score(y_test, y_pred, labels=nb_clf.classes_, average='weighted')
        nb_clf_f1_macro[i] = f1_score(y_test, y_pred, labels=nb_clf.classes_, average='macro')
        nb_clf_f1_micro[i] = f1_score(y_test, y_pred, labels=nb_clf.classes_, average='micro')
        ...
        svc_preds.append(y_pred)
        y_unk_pred = svc_clf.predict(X_unk_test)
        svc_unk_preds.append(y_unk_pred)

        plot_learning_curves(X_train, y_train, X_test, y_test, clf=svc_clf, train_marker='o', test_marker='^',
                             scoring='misclassification error', print_model=True, style='fivethirtyeight',
                             legend_loc='upper left')
        plt.show()

        svc_accuracy[i] = accuracy_score(y_test, y_pred)
        svc_f1_weighted[i] = f1_score(y_test, y_pred, labels=svc_clf.classes_, average='weighted')
        svc_f1_macro[i] = f1_score(y_test, y_pred, labels=svc_clf.classes_, average='macro')
        svc_f1_micro[i] = f1_score(y_test, y_pred, labels=svc_clf.classes_, average='micro')
        ...
        logicR_preds.append(y_pred)
        y_unk_pred = regr.predict(X_unk_test)
        logicR_unk_preds.append(y_unk_pred)

        logicR_accuracy[i] = accuracy_score(y_test, y_pred)
        logicR_f1_weighted[i] = f1_score(y_test, y_pred, labels=regr.classes_, average='weighted')
        logicR_f1_macro[i] = f1_score(y_test, y_pred, labels=regr.classes_, average='macro')
        logicR_f1_micro[i] = f1_score(y_test, y_pred, labels=regr.classes_, average='micro')

        plot_learning_curves(X_train, y_train, X_test, y_test, clf=regr, train_marker='o', test_marker='^',
                             scoring='accuracy', print_model=True, style='fivethirtyeight', legend_loc='upper left')
        plt.show()
        plt.savefig("logistic_regression.svg")

        if config.print_status == 'Y':
            print("voting classifier CV: ", i + 1)

        voting_clf = VotingClassifier(estimators=[('rf', rf), ('lr', regr), ('svc', svc_clf)], voting='hard')

        voting_clf.fit(X_train, y_train)
        y_pred = voting_clf.predict(X_test)
        voting_clf_preds.append(y_pred)
        y_unk_pred = voting_clf.predict(X_unk_test)
        voting_clf_unk_preds.append(y_unk_pred)

        voting_clf_accuracy[i] = accuracy_score(y_test, y_pred)
        voting_clf_f1_weighted[i] = f1_score(y_test, y_pred, labels=voting_clf.classes_, average='weighted')
        voting_clf_f1_macro[i] = f1_score(y_test, y_pred, labels=voting_clf.classes_, average='macro')
        voting_clf_f1_micro[i] = f1_score(y_test, y_pred, labels=voting_clf.classes_, average='micro')

        plot_learning_curves(X_train, y_train, X_test, y_test, clf=voting_clf, train_marker='o', test_marker='^',
                             scoring='accuracy', print_model=True, style='fivethirtyeight', legend_loc='upper left')
        plt.show()
        plt.savefig("logistic_regression.svg")
    ...
    cvs = np.zeros((len(output_names), 8))
    for i in range(len(output_names)):
        cvs[i][0] = vars()['rf_' + output_names[i]].mean()
        cvs[i][1] = vars()['ovr_' + output_names[i]].mean()
        cvs[i][2] = vars()['svc_' + output_names[i]].mean()
        cvs[i][3] = vars()['logicR_' + output_names[i]].mean()
        cvs[i][4] = vars()['nb_clf_' + output_names[i]].mean()
        cvs[i][5] = vars()['voting_clf_' + output_names[i]].mean()
        cvs[i][6] = vars()['knn_' + output_names[i]].mean()
        cvs[i][7] = vars()['dt_' + output_names[i]].mean()

    metrics = pd.DataFrame(data=cvs, index=metric_names, columns=['RF', 'OVR', 'SVM', 'logicR', 'Naive Bayes','Voting Classifier', "KNN",'Decistion Tree'])
...
def main():
    ...
    spacedict = {'criterion': ['gini', 'entropy'],
                 'max_features': np.arange(2, 22, 2) if config.max_features is None else np.arange(
                     config.max_features[0],
                     config.max_features[1], config.max_features[2]),
                 'max_depth': np.arange(5, 35, 5) if config.max_depth is None else np.arange(config.max_depth[0],
                                                                                             config.max_depth[1],
                                                                                             config.max_depth[2]),
                 'min_samples_split': np.arange(2, 20, 2) if config.min_samples_split is None else np.arange(
                     config.min_samples_split[0],
                     config.min_samples_split[1], config.min_samples_split[2]),
                 'n_estimators': np.arange(10, 250, 25) if config.n_estimators is None else np.arange(
    ...
    plot_learning_curves(X_overall, y_overall, X_overall, y_overall, clf=rf_final, scoring='accuracy')
    plt.show()
    ...
    ovr_unk_preds_frame = pd.DataFrame(ovr_unknown_preds, index=X_unk_overall.index,
                                       columns=['OVRFullModelPredLOINCKey'])

    if config.run_cv == 'Y':
        X0 = X_cvs.copy()
        unknowns_analysis = X_unk_cvs.copy()

    X_final = X0.merge(rf_preds_frame, how='inner', left_index=True, right_index=True).merge(ovr_preds_frame,
                                                                                             how='inner',
                                                                                             left_index=True,
                                                                                             right_index=True)
    unknowns_final = unknowns_analysis.merge(rf_unk_preds_frame, how='inner', left_index=True, right_index=True).merge(
        ovr_unk_preds_frame, how='inner', left_index=True, right_index=True)
    ...
    if config.run_cv == 'Y':
        ...
        X_final[['RFCVPredLOINC', 'RFCVLOINC', 'OVRCVPredLOINC', 'OVRCVLOINC']] = loinc_coder.inverse_transform(
            X_final[['RFCVPredLOINC', 'RFCVLOINC', 'OVRCVPredLOINC', 'OVRCVLOINC']])
        unknowns_final[['RFCVPredLOINC', 'OVRCVPredLOINC']] = loinc_coder.inverse_transform(
            unknowns_final[['RFCVPredLOINC', 'OVRCVPredLOINC']])

    X_final[[config.loinc_col, 'LOINC_KEY', 'RFFullModelPredLOINCKey', 'OVRFullModelPredLOINCKey',
             'RFFullModelFinalLOINCKey',
             'OVRFullModelFinalLOINCKey']] = loinc_coder.inverse_transform(X_final[[config.loinc_col,
                                                                                    'LOINC_KEY',
                                                                                    'RFFullModelPredLOINCKey',
                                                                                    'OVRFullModelPredLOINCKey',
                                                                                    'RFFullModelFinalLOINCKey',
                                                                                    'OVRFullModelFinalLOINCKey']])
    unknowns_final[[config.loinc_col, 'RFFullModelPredLOINCKey', 'OVRFullModelPredLOINCKey',
                    'LOINC_KEY']] = loinc_coder.inverse_transform(
        unknowns_final[[config.loinc_col, 'RFFullModelPredLOINCKey', 'OVRFullModelPredLOINCKey', 'LOINC_KEY']])
    ...
    X_final[cols_to_transform] = X_final[cols_to_transform].apply(
        lambda x: label_encoder_dict[x.name].inverse_transform(x))
    unknowns_final[cols_to_transform] = unknowns_final[cols_to_transform].apply(
        lambda x: label_encoder_dict[x.name].inverse_transform(x))
