...
def get_loinc_groups():
    ...
    raw_loinc_dat = raw_loinc_dat[(raw_loinc_dat.CLASSTYPE == 1) | (raw_loinc_dat.CLASSTYPE == 2)].reset_index(
        drop=True)
    ...
    raw_loinc_mults = raw_loinc_dat.merge(raw_loinc_mults, how='inner', left_on=['COMPONENT', 'PROPERTY', 'TIME_ASPCT',
                                                                                 'SYSTEM', 'SCALE_TYP'],
                                          right_on=['COMPONENT', 'PROPERTY', 'TIME_ASPCT', 'SYSTEM', 'SCALE_TYP']) \
        .drop_duplicates() \
...
def quant_nans(group):
    ...
    elif group['METHOD_TYP'].isnull().any() and group[(group.METHOD_TYP.isnull()) & (group.STATUS == 'ACTIVE')].shape[
        0] == 1:
        _ = group['CANDIDATE_KEYS'] = 1
        _ = group['LOINC_KEY'] = group[(group.METHOD_TYP.isnull()) & (group.STATUS == 'ACTIVE')].LOINC_NUM.values[0]
    elif group['METHOD_TYP'].isnull().any() and \
            group[(group.METHOD_TYP.isnull()) & (group.STATUS == 'DISCOURAGED')].shape[0] == 1:
        _ = group['CANDIDATE_KEYS'] = 1
        _ = group['LOINC_KEY'] = group[(group.METHOD_TYP.isnull()) & (group.STATUS == 'DISCOURAGED')].LOINC_NUM.values[
            0]
...
def get_loinc_synonyms():
    ...
    raw_loinc_mults = raw_loinc_mults.groupby(['COMPONENT', 'PROPERTY', 'TIME_ASPCT', 'SYSTEM', 'SCALE_TYP']).apply(
        quant_nans)
